...
<div className="mt-10 bg-white shadow-2xl rounded-3xl overflow-hidden border border-gray-200 flex flex-col md:flex-row p-8 md:p-10 gap-8 my-10">
  ...
  <div className="md:w-2/3 space-y-5">
    {/* Title and Author */}
    <div>
      <h2 className="text-3xl font-extrabold text-gray-900">{bookName}</h2>
    ...
    <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
      ...
      <p className="text-gray-600 italic text-sm leading-relaxed">
        "{review?.slice(0, 180)}..."
        <span className="text-indigo-500 cursor-pointer ml-1 hover:underline">Show More</span>
      </p>
    </div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm flex-3">
      <div className="flex items-center gap-2 bg-purple-50 rounded-lg px-3 py-2">
        ...
        <p><span className="font-medium"></span> {category}</p>
      </div>
      <div className="flex items-center gap-2 bg-blue-50 rounded-lg px-3 py-2">
        <span className="text-blue-600">📘</span>
        <p><span className="font-medium"></span> {totalPages}</p>
      </div>
      <div className="flex items-center gap-2 bg-yellow-50 rounded-lg px-3 py-2">
        <span className="text-yellow-600">🏢</span>
        <p><span className="font-medium"></span> {publisher}</p>
      </div>
      <div className="flex items-center gap-2 bg-green-50 rounded-lg px-3 py-2">
        <span className="text-green-600">📅</span>
        <p><span className="font-medium"></span> {yearOfPublishing}</p>
      </div>
    </div>
    ...
    <div className="flex items-center gap-2 bg-pink-50 rounded-lg px-3 py-2 inline-block">
      ...
      <p className="font-medium text-gray-700"> <span className="text-pink-600 font-semibold">{rating}</span></p>
    </div>
